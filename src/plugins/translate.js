...
var translate = {
    ...
    */
	...
	selectLanguageTag: {
        ...
        */
		...
		changeLanguageBeforeLoadOfflineFile: (_path) => {},
        ...
        */
		export: () => {
			...
			for (var uuid in translate.nodeQueue) {
				...
				var _queueValue = translate.nodeQueue[uuid];
	...
	listener: {
        ...
        */
		...
		addListener: () => {
			...
			// 当观察到变动时执行的回调函数
			translate.listener.callback = (mutationsList, _observer) => {
				...
				for (
					...
					for (var task_index = 0; task_index < tasks.length; task_index++) {
						...
						var analyseSet = translate.element.nodeAnalyse.set(
							...
							task.resultText,
							task.attribute,
							typeof task.participles === "undefined" ? [] : task.participles,
						);
    ...
    */
		...
		changeLanguage: {
            ...
            */
			...
			resetAfter_Trigger: (data) => {
				var _isNextExecute = true; //是否继续向下执行，true则是继续执行，false则是不继续执行。
				for (
					var i = 0;
		...
		refresh: () => {
			...
			for (const key of translate.node.data.keys()) {
				...
				if (key.nodeType === Node.ELEMENT_NODE) {
					...
					// 属性节点（如placeholder）
					isValidNode = key.ownerElement?.isConnected;
		...
		nodeAnalyse: {
            ...
            */
			analyseReplaceBefore_DateToTranslateNode: (
				...
				var translateNode = null; //当前操作的，要记录入 translate.node 中的，进行翻译的node
				var _translateNode_attribute = ""; //当前操作的是node中的哪个attribute，如果没有是node本身则是空字符串
				...
				if (typeof attribute === "string" && attribute.length > 0) {
					...
					if (attribute !== null && attribute === "value") {
						...
						if (
							...
							translateNode = node;
							_translateNode_attribute = "value";
						}
					}
					if (translateNode === null) {
						translateNode = node.getAttributeNode(attribute);
						_translateNode_attribute = attribute;
					}
				} else {
					//操作的就是node本身
					translateNode = node;
			...
			if (
				...
				for (var attributeName_index in translate.element.tagAttribute[
					...
					if (
						...
						_DOMPropOrHTMLAttr = "DOMProperty";
						isInputValue = true;
					} else {
						/*
						 ...
						 * DOMProperty : DOM属性
						 */
						var _DOMPropOrHTMLAttr = "HTMLAtrribute";
     ...
     */
	addNodeToQueue: (uuid, node, _text, attribute) => {
		...
		//console.log('\t\t'+text);
		//取要翻译字符的hash
		var _key = translate.util.hash(text);
		/*
    ...
    */
	...
	language: {
        ...
        */
		 ...
		 * chatstr  : 当前字符，如  h
		 */
		analyse: (language, langStrs, upLangs, _upLangsTwo, charstr) => {
	...
	util: {
        ...
        */
		versionStringToInt: (versionString) => {
			...
			var result = 0;
			result = Number.parseInt(vs[0], 10) * 1000 * 1000 + result;
			result = Number.parseInt(vs[1], 10) * 1000 + result;
			result = Number.parseInt(vs[2], 10) + result;
	...
	service: {
        ...
        */
		...
		edge: {
			 ...
			 */
			translate: (_path, data, func, abnormalFunc) => {
	...
	request: {
        ...
        */
		speedDetectionControl: {
			...
			getHostQueue: () => {
				if (translate.request.speedDetectionControl.hostQueue.length === 0) {
                    ...
                    */
					...
					if (Date.now() - lasttime > updateTime) {
						translate.request.speedDetectionControl.checkResponseSpeed();
					}
				}
            ...
            */
			checkResponseSpeed_Storage: (host, time) => {
				...
				translate.storage.set("speedDetectionControl_lasttime", Date.now());
		...
		post: function (path, data, func, abnormalFunc) {
			...
			// ------- edge start --------
			var _url = translate.request.getUrl(path);
            ...
            */
			...
			if (translate.ignore.text.length > 0) {
				...
				for (var tai = 0; tai < texts.length; tai++) {
					var _isFind = false; //是否发现匹配进行拆分了，true是
    ...
    */
	visual: {
        ...
        */
		adjustTranslationSpacesByLastNodequeueUuid: (_uuid) => {
			var uuid = "";
		...
		webPageLoadTranslateBeforeHiddenText: (config) => {
			...
			if (
                ...
                */
				...
				translate.lifecycle.execute.renderFinish.push((uuid, _to) => {
    ...
    */
	time: {
		...
		execute: {
            ...
            */
			start: () => {
				...
				//翻译开始
				translate.lifecycle.execute.start.push((uuid, _to) => {
					if (typeof translate.time.execute.data[uuid] === "undefined") {
						translate.time.execute.data[uuid] = {};
					}
					translate.time.execute.data[uuid].startTime = Date.now();
				});
                ...
                */
				...
				//翻译完成（渲染全部语种都完成）
				translate.lifecycle.execute.renderFinish.push((uuid, _to) => {
					translate.time.execute.data[uuid].finishTime = Date.now();
		...
		log: (remark) => {
			...
			var timestamp = Date.now(); // 例如：1725053445123
    ...
    */
	init: () => {
		...
		translate.lifecycle.execute.start.push((_uuid, _to) => {
			//只在第一次触发时，才会做一些初始化
